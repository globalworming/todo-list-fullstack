...
import com.headissue.compliance.servlet.*;
...
public class ServletApplication {
  ...
  private static final ManagedChannel todoServiceChannel =
      ChannelFactory.buildChannel("TODO_SERVICE_HOST", "localhost:8081");

  public static void main(String[] args) {
    ToDoServiceGrpc.ToDoServiceBlockingStub toDoServiceStub =
        ToDoServiceGrpc.newBlockingStub(todoServiceChannel);
  ...
  public static void test(
      HealthGrpc.HealthBlockingStub todoHealthStub,
      ToDoServiceGrpc.ToDoServiceBlockingStub toDoServiceStub) {
  ...
  private static void start(
    ...
    ServletHolder auth = new ServletHolder(new AuthServlet(authServiceStub));
    servletHandler.addServlet(auth, "/auth");
    ServletHolder me = new ServletHolder(new MeServlet(authServiceStub));
    servletHandler.addServlet(me, "/me");
